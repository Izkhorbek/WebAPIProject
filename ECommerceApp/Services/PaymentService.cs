...
namespace ECommerceApp.Services
{
    public class PaymentService
    {
                ...
                try
                {
                    ...
                    if (order.Payment != null)
                    {
                        ...
                        if (order.Payment.Status == PaymentStatus.Failed && order.OrderStatus == OrderStatus.Pending)
                        {
                            ...
                            payment.TransactionId = null; // Clear previous transaction
        ...
        //Generate a unique 12-characters transaction ID
        ...
        {
            // Retriece the order with its related customer, address, payment, and order items (with products)
            var order = await _context.Orders
                .AsNoTracking()
            ...
            // Semd the email using the EmailService
